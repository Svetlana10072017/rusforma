...
class CategoryRequest extends FormRequest
{
    ...
    {
         //правила для заполения
            $rules = [
                'code' => 'required|min:4|max:255|unique:categories,code',//уникальное поле
                'name' => 'required|min:5|max:255',

            ];

            if ($this->route()->named('categories.update')) {//для проверки уникальности поля
